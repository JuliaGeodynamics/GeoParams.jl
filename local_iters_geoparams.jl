...
function Local_Iterations()
    ...
    v = (v_dis, v_el) # dislocation and elasticity are in serie
    ...
    # Local Iterations
    @time "iterations" while iter < 5  # Newton
        iter = iter + 1
        f = εII_ve - strain_rate_circuit(τII, v, args)
        dfdτII = 0.0 - dεII_dτII(τII, v_dis, args) - dεII_dτII(τII, v_el, args) # yet to be generalized
        τII = τII - f / dfdτII
        println("Residual = $(f); τII = $(τII)")
