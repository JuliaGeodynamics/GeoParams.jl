...
function compute_heatcapacity!(
    cp_array::AbstractArray{_T}, s::ConstantHeatCapacity{_T}; kwargs...
) where {_T}
...
"""
function compute_heatcapacity!(
    cp_array::AbstractArray{_T,N},
    s::T_HeatCapacity_Whittington{_T};
...
for myType in (:ConstantHeatCapacity, :T_HeatCapacity_Whittington)
    @eval begin
        ...
        function compute_heatcapacity!(
            k::AbstractArray{_T,N}, s::$(myType){_T}, args
        ) where {_T,N}
            return compute_heatcapacity!(k, s; args...)
        end
    end
end
