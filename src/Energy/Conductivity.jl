...
"""
function compute_conductivity!(
    k::AbstractArray{_T,N},
    s::T_Conductivity_Whittington{_T};
    T::AbstractArray{_T,N},
    kwargs...,
) where {_T,N}
    if T isa Quantity
        @unpack_units a0, a1, b0, b1, c0, c1, molmass, Tcutoff, rho, d, e, f, g = s
...
function compute_conductivity!(
    k::AbstractArray{_T,N},
    s::T_Conductivity_Whittington_parameterised{_T};
    T::AbstractArray{_T,N},
    kwargs...,
) where {_T,N}
    if T isa Quantity
...
for myType in (
    ...
    @eval begin
        ...
        function compute_conductivity!(k::AbstractArray, s::$(myType), args)
            return compute_conductivity!(k, s; args...)
        end
    end
end
