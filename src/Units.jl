...
for op in (:+,:-,:*,:/)
    ...
    @eval Base.$op(x::GeoUnit{T1,U1}, y::GeoUnit{T2,U2}) where {T1,T2,U1,U2} = GeoUnit($(op)(Value(x), Value(y)))
    ...
    # Broadcasting
    @eval Base.broadcasted(::typeof($(op)), A::GeoUnit, B::AbstractArray) = broadcast($(op), NumValue(A), B)
    @eval Base.broadcasted(::typeof($(op)), A::AbstractArray, B::GeoUnit) = broadcast($(op), A, NumValue(B))
    @eval Base.broadcasted(::typeof($(op)), A::GeoUnit, B::AbstractArray{<:Quantity}) = broadcast($(op), Value(A), B)
    @eval Base.broadcasted(::typeof($(op)), A::AbstractArray{<:Quantity}, B::GeoUnit) = broadcast($(op), A, Value(B))
end
