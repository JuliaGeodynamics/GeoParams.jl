...
@inline function compute_εII(
    s::ConstantElasticity, τII::_T; τII_old=zero(precision(a)), dt=one(precision(a)), kwargs...
) where {_T}
    @unpack_val G = s
...
@inline function compute_τII(
    s::ConstantElasticity, εII::_T; τII_old=zero(precision(s)), dt=one(precision(a)), kwargs...
) where {_T}
    @unpack_val G = s
    τII = precision(a)(2) * G * dt * εII + τII_old
