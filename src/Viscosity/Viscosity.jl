...
@generated function compute_viscosity_εII(v::NTuple{N, AbstractMaterialParamsStruct}, phase, args) where N
    quote
        Base.@nexprs $N i -> i == phase && (return compute_viscosity_εII(v.CompositeRheology[i], args...))
        return 0.0
    end
end

@generated function compute_viscosity_τII(v::NTuple{N, AbstractMaterialParamsStruct}, phase, args) where N
    quote
        Base.@nexprs $N i -> i == phase && (return compute_viscosity_τII(v.CompositeRheology[i], args))
        return 0.0
    end
end
...
# single phase versions
compute_elastoviscosity_εII(v::MaterialParams, args::Vararg{Any, N}) where {N} = compute_elastoviscosity_εII(v.CompositeRheology[1], args...)
compute_elastoviscosity_τII(v::MaterialParams, args::Vararg{Any, N}) where {N} = compute_elastoviscosity_τII(v.CompositeRheology[1], args...)

# multi-phase versions
@generated function compute_elastoviscosity_εII(v::NTuple{N, AbstractMaterialParamsStruct}, phase, args) where N
    quote
        Base.@nexprs $N i -> i == phase && (return compute_elastoviscosity_εII(v.CompositeRheology[i], args))
        return 0.0
    end
end

@generated function compute_elastoviscosity_τII(v::NTuple{N, AbstractMaterialParamsStruct}, phase, args) where N
    quote
        Base.@nexprs $N i -> i == phase && (return compute_elastoviscosity_τII(v.CompositeRheology[i], args))
        return 0.0
    end
end