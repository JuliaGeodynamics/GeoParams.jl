...
function compute_param(fn::Function, MatParam::AbstractVector{AbstractMaterialParamsStruct}, Phase::Int64, args)  
    return compute_param(fn, Tuple(MatParam), Phase, args)
end

@inline function compute_param!(fn::Function, rho::AbstractArray, MatParam::NTuple{N,AbstractMaterialParamsStruct}, Phases::AbstractArray{<:Integer, ndim}, args) where {ndim,N}
...
function compute_param!(fn::Function, rho::AbstractArray, MatParam::AbstractVector{AbstractMaterialParamsStruct}, Phases::AbstractArray{<:Integer, ndim}, args) where ndim
    compute_param!(fn, rho, Tuple(MatParam), Phases, args)
end

#Phase PhaseRatios

function compute_param!(fn::Function, rho::AbstractArray{_T, N}, MatParam::NTuple{K,AbstractMaterialParamsStruct}, PhaseRatio::AbstractArray{_T, M}, args) where {_T, N, M, K}
    if M!=(N+1)
...
function compute_param_times_frac(fn::Function, PhaseRatios::NTuple{N, T}, MatParam::NTuple{N,AbstractMaterialParamsStruct}, argsi) where {N, T}
