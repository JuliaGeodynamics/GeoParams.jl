...
@generated function compute_εII(
    v::Parallel{V, N}, τII::T, args) where {V, T, N}
    quote
        η = zero($T)
...
#=
"""
    compute_εII(v::NTuple{N,AbstractConstitutiveLaw}, τII, args; tol=1e-6, verbose=false, n=1)
...
Performs local iterations versus stress for a given total applied strain rate 
"""
...
@inline function local_iterations_τII(
    ...
    # Initial guess of τII (harmonic average)
...
@generated function dτII_dεII(
    ...
    quote
        Base.@_inline_meta
        val = zero(_T)
        Base.Cartesian.@nexprs $N i -> val += dτII_dεII(v.elements[i], εII, args)
        return val
    end
end
=#
...
#dεII_dτII(v::CompositeRheology, τII, args) = dεII_dτII(v.elements,τII, args)
...
@generated function compute_εII_harmonic(
    ...
    quote
        out = zero($_T)
        Base.Cartesian.@nexprs $N i ->
            out += inv(compute_εII(v[i], TauII, args))
        return inv(out)
    end
end
=#

#function compute_εII_harmonic(v::CompositeRheology{T,N}, TauII::_T, args) where {T,N,_T}
#    return compute_εII_harmonic(v.elements, TauII, args)
#end

#=
"""
    compute_τII_harmonic(v::NTuple, EpsII, args)
...
@inline @generated function compute_τII_harmonic(
    ...
    quote
        out = zero(_T)
        Base.Cartesian.@nexprs $N i ->
            out += 1/compute_τII(v[i], EpsII, args)
        out = 1/out
    end
end
=#
...
@generated function strain_rate_circuit(
    v::NTuple{N,AbstractConstitutiveLaw}, τII::T, args
) where {N, T}
    quote
        Base.@_inline_meta
        εII = zero($T)
        Base.Cartesian.@nexprs $N i -> εII += strain_rate_circuit(v[i], τII, args)
        return εII
    end
end
...
@generated function stress_circuit(
    v::Parallel{T, N}, εII::_T, args
) where {N,T,_T}
    quote
        Base.@_inline_meta
        τ = zero(_T)
        Base.Cartesian.@nexprs $N i ->
            τ += compute_τII(v.elements[i], εII, args)
        return τ
    end
end
=#