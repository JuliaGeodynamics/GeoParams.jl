...
function cold_dry_olivine_Hirth_2003()
    data = GrainBoundarySliding(;
        ...
        A = 1.0e-33 * (6500.0Pa^(-7 // 2) * m^(2) * s^(-1)),  # material specific rheological parameter
...
function hot_dry_olivine_Hirth_2003()
    data = GrainBoundarySliding(;
        ...
        n = 3.5NoUnits,                         # power-law exponent
        p = -2.0NoUnits,                        # grain size exponent
        A = 1.0e-33 * (4.7e10Pa^(-7 // 2) * m^(2) * s^(-1)), # material specific rheological parameter
...
function test_GBS()
    data = GrainBoundarySliding(;
        ...
        n = 3.5NoUnits,                         # power-law exponent
        p = -2.0NoUnits,                        # grain size exponent
        A = 1.0e-21 * (1.506190693026593e2Pa^(-7 // 2) * m^(2) * s^(-1)),    # material specific rheological parameter
        E = 600.0e3J / mol,                        # activation energy
        V = 18.0e-6m^3 / mol,                       # activation Volume
        Apparatus = AxialCompression,
    )
