...
export DislocationCreep, SetDislocationCreep, dεII_dτII, dτII_dεII
...
function DislocationCreep(args...)
...
@inline function computeCreepLaw_EpsII(
    TauII, a::DislocationCreep; P::_R, T::_R, f::_R, args...
) where {_R<:Real}
...
@inline function computeCreepLaw_TauII(
    EpsII, a::DislocationCreep; P::_R, T::_R, f::_R, args...
) where {_R<:Real}
    @unpack_val n, r, A, E, V, R = a
...
function show(io::IO, g::DislocationCreep)
    return print(
        io,
        "DislocationCreep: Name = $(String(collect(g.Name))), n=$(g.n.val), r=$(g.r.val), A=$(g.A.val), E=$(g.E.val), V=$(g.V.val), Apparatus=$(g.Apparatus)",
    )
end
#-------------------------------------------------------------------------
