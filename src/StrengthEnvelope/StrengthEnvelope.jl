...
```
"""
function StrengthEnvelopeComp(MatParam::NTuple{N, AbstractMaterialParamsStruct}, Thickness::Vector{U}, TempType::AbstractTempStruct = LinTemp(0C, 800C), ε = 1.0e-15 / s, nz::Int64 = 101) where {N, U}

    # hardcoded input
    g = 9.81m / s^2

    # nondimensionalize
    CharDim = GEO_units(length = 10km, temperature = 1000C, stress = 10MPa, viscosity = 1.0e20Pas)
...
function StrengthEnvelopePlot()
    return println("Load GLMakie to use this function: \n julia> using GLMakie")
end
