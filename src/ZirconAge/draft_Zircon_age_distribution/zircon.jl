...
"""
function zircon_fraction(T,zircon_max_frac)
	A = (1.62.-1.8*(10^4)*exp.((.-10^4)./(T.+273))).*max_x_zr 
	A[A .<= 0.0] .= 0.0
...
"""
function loess_fit_zircon_sat(	Tsol,
								Tcal_step,
								Tcal_max, 
								Tsat,
								max_x_zr	)

	# Get cummulative Zircon fracion (after Tierney et al., 2016; Geology)
...
# Clarified way to define the zircon growth as a minimum time within T saturation range (This is what the method used in the R script, boils down)
...
# Parameteres for Zircon statistical analysis
...
n_paths			= size(Tt_paths,2)-1							# -1 ro remove first column that corresponds to the age
...
n_zircon_N_fit = loess_fit_zircon_sat(Tsol,Tcal_step,Tcal_max, Tsat, max_x_zr)
