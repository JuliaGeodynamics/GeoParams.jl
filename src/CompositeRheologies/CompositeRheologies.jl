...
export compute_εII_harmonic, compute_τII_AD, isplastic, compute_p_τII, local_iterations_εvol, compute_p_harmonic
...
function nondimensionalize(MatParam::Union{Parallel, CompositeRheology}, g::GeoUnits{TYPE}) where {TYPE}
    ...
    for i in 1:length(MatParam.elements)
...
function computeViscosity_εII(v::T, εII::_T, args; tol = 1.0e-6, verbose = false) where {T <: AbstractConstitutiveLaw, _T}
...
function computeViscosity_εII_AD(v::T, εII::_T, args; tol = 1.0e-6, verbose = false) where {T <: AbstractConstitutiveLaw, _T}
