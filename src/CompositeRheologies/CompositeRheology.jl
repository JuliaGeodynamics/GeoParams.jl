...
"""
@inline compute_εII(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(compute_εII(vi, τII, args)), v.elements)
@inline compute_εII(v::CompositeRheology, τII::Quantity, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(compute_εII(vi, τII, args)), v.elements)
...
function compute_εII_AD(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false)
...
"""
compute_εvol(v::CompositeRheology, p, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(compute_εvol(vi, p, args)), v.elements)
compute_εvol(v::CompositeRheology, p::Quantity, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(compute_εvol(vi, p, args)), v.elements)

# COMPUTE DEVIATORIC STRESS AND PRESSURE
function compute_τII(v::CompositeRheology{T, N, 0}, εII, args; tol = 1.0e-6, verbose = false) where {T, N}
...
"""
function compute_τII(v::CompositeRheology, εII, args; tol = 1.0e-6, verbose = false, τ_initial = nothing, ε_init = nothing)
...
function compute_τII_AD(v::CompositeRheology, εII, args; tol = 1.0e-6, verbose = false)
...
dεII_dτII(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(dεII_dτII(vi, τII, args)), v.elements)
...
"""
dεII_dτII_elements(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(dεII_dτII_nonparallel(vi, τII, args)), v.elements)
...
"""
compute_εII_nonplastic(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(_compute_εII_nonplastic(vi, τII, args)), v.elements)
...
Harmonic average of stress of all elements in a `CompositeRheology` structure that are not || elements
"""
function compute_p_harmonic(v::CompositeRheology, EpsVol, args; tol = 1.0e-6, verbose = false)
...
"""
function compute_εII_harmonic(v::Union{Parallel, CompositeRheology}, TauII, args; tol = 1.0e-6, verbose = false)
...
"""
compute_εII_elements(v::CompositeRheology, τII, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(_compute_εII_nonparallel(vi, τII, args)), v.elements)
...
Sums the strainrate of all non-parallel and non-plastic elements in a `CompositeRheology` structure. Mostly internally used for jacobian iterations.
"""
compute_εvol_elements(v::CompositeRheology, p, args; tol = 1.0e-6, verbose = false) = nreduce(vi -> first(_compute_εvol_elements(vi, p, args)), v.elements)
