...
function Melt_Dy_Holycross2018_rhyolitic_highH2O()
    data = DiffusionData(
        ...
        Species="Dy",  # element or species being diffused
        ...
        D0=(10^(-4.60))u"m^2 / s",  # pre-exponential factor (log10(-3.21) in the original paper
        log_D0_1σ=1.04 * 2.303NoUnits,  # uncertainty reported as log10(D0) in the original paper
        Ea=(176.47)u"kJ/mol",  # activation energy
        Ea_1σ=(23.27)u"kJ/mol",  # uncertainty at 1σ of the activation energy
...
function Melt_Dy_Holycross2018_rhyolitic_mediumH2O()
    data = DiffusionData(
        ...
        Fluid="Hydrated",  # Fluid condition (e.g., anhydrous) during the experiment
        D0=(10^(-4.90))u"m^2 / s",  # pre-exponential factor (log10(-3.21) in the original paper
        log_D0_1σ=0.32 * 2.303NoUnits,  # uncertainty reported as log10(D0) in the original paper
        Ea=(181.21)u"kJ/mol",  # activation energy
        Ea_1σ=(8.23)u"kJ/mol",  # uncertainty at 1σ of the activation energy
        Charge=3,  # charge of the cation
