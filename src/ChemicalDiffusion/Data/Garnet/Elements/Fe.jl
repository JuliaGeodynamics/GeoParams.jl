...
Diffusion data of Fe in garnet calibrated using natural garnets. From Carlson (2006) (https://doi.org/10.2138/am.2006.2043).
"""
function Grt_Fe_Carlson2006()
    data = DiffusionData(
        Name = "Fe diffusion in Garnet (C-O2) | Carlson (2006)",
        Phase = "Garnet",  # name of the mineral
        Formula = "X3Y2(SiO4)3",  # chemical formula of the mineral
        Species = "Fe",  # element or species being diffused
        Orientation = "Isotropic",  # Crystal orientation from the diffusion experiment
        ...
        D0 = exp(-17.638)u"m^2/s",  # pre-exponential factor
        log_D0_1σ = 0.776NoUnits,
        Ea = 264.55u"kJ/mol",  # activation energy
        Ea_1σ = 4.63u"kJ/mol",  # uncertainty at 1σ of the activation energy
        ΔV = 13.568cm^3 / mol,  # activation volume
...
Diffusion data of Fe in garnet calibrated using compilation of experiments data from Cygan and Lasaga (1985), Elphick et al. (1985), Loomis et al. (1985), Chakraborty and Ganguly (1992), Chakraborty and Rubie (1996), Ganguly et al. (1998), Borinski et al. (2012), Vielzeuf et al. (2007), Vielzeuf and Saul (2011) and Borinski et al. (2012).
From Chu and Ague (2015) (https://doi.org/10.1007/s00410-015-1175-y).
"""
function Grt_Fe_Chu2015()
    data = DiffusionData(
        Name = "Fe diffusion in Garnet (C-O2) | Chu and Ague (2015)",
        Phase = "Garnet",  # name of the mineral
        Formula = "X3Y2(SiO4)3",  # chemical formula of the mineral
        Species = "Fe",  # element or species being diffused
        Orientation = "Isotropic",  # Crystal orientation from the diffusion experiment
        Crystallography = "Isometric",  # Crystallographic system of the mineral
        D0 = (10^(-7.98))u"m^2/s",  # pre-exponential factor
