...
function naverage(x::NTuple{N,Any}) where N
    ntuple( i -> naverage(x[i]), Val(N))
end

naverage(x::NTuple{N,T}) where {N,T} = sum(xi for xi in x)/N
naverage(x::T) where {T<:Number} = x
