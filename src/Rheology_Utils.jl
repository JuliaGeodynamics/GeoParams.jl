...
"""
function time_τII_0D(v::Union{CompositeRheology, Tuple, Parallel}, εII::Number, args; t = (0.0, 100.0), τ0 = 0.0, nt::Int64 = 100, verbose = true)
...
"""
function time_τII_0D!(τ_vec::Vector{NTuple{N, T}}, τII_vec::Vector{T}, v::Union{CompositeRheology, Tuple, Parallel}, ε_vec::Vector{NTuple{N, T}}, args, t_vec::AbstractVector{T}; verbose = false) where {N, T}
    ...
    for i in 2:nt
        dt = t_vec[i] - t_vec[i - 1]
        args = merge(args, (; τII_old = 0, dt = dt))
        τij, τII = compute_τij(v, ε_vec[i - 1], args, τ_vec[i - 1])
...
The initial stress `τ0`, the time range `t` and the number of timesteps `nt` can be modified 
"""
function time_p_τII_0D(v::Union{CompositeRheology, Tuple, Parallel}, εII::Number, εvol::Number, args; t = (0.0, 100.0), τ0 = 0.0, nt::Int64 = 100, verbose = true)
    t_vec = range(t[1], t[2], length = nt)
    τ_vec = zero(t_vec)
    P_vec = zero(t_vec)
    εII_vec = zero(t_vec) .+ εII
