...
for (name, backend) in zip(pkg, backends)
    @testset "$name compatibility with: density" begin
        ρ = ConstantDensity()
        @test AD.derivative(backend, x -> compute_density(ρ, (; T = x, P = 1.0e9)), T)[1] == 0
        @test AD.derivative(backend, x -> compute_density(ρ, (; T = 1.0e3, P = x)), P)[1] == 0

        ρ = PT_Density()
        @test AD.derivative(backend, x -> compute_density(ρ, (; T = x, P = 1.0e9)), T)[1] ≈ -0.087
        @test AD.derivative(backend, x -> compute_density(ρ, (; T = 1.0e3, P = x)), P)[1] == 2.9e-6

        ρ = Compressible_Density()
        @test AD.derivative(backend, x -> compute_density(ρ, (; T = 1.0e3, P = x)), P)[1] ≈ 7.8830173e-6
    ...
    @testset "$name compatibility with: Diffusion" begin
        ...
        args = (; T = T, P = P)
        TauII = 1.0e6
        @test AD.derivative(backend, x -> compute_εII(p, TauII, (; T = x, P = P)), T)[1] ≈ 2.1319560317213205e-34
        @test AD.derivative(backend, x -> compute_εII(p, TauII, (; T = T, P = x)), P)[1] ≈ -1.0571682801923905e-41
    ...
    @testset "$name compatibility with: Dislocation" begin
        ...
        @test AD.derivative(backend, x -> compute_τII(p, εII, (; T = T, P = x)), P)[1] ≈ 0.0016838054
    end
    ...
    @testset "$name compatibility with: CompositeRheology" begin
        ...
        @test AD.derivative(backend, x -> compute_τII(c1, εII, (; T = x, P = P, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), T)[1] ≈ -0.44160212209877125
        @test AD.derivative(backend, x -> compute_τII(c1, εII, (; T = T, P = x, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), P)[1] ≈ 1.5937520196046195e-8
        @test AD.derivative(backend, x -> compute_εII(c1, τII, (; T = x, P = P, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), T)[1] ≈ 2.6551361627056197e-22
        @test AD.derivative(backend, x -> compute_εII(c1, τII, (; T = T, P = x, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), P)[1] ≈ -9.58244630149972e-30
        # test linear rheology
        @test iszero(AD.derivative(backend, x -> compute_τII(c2, εII, (; T = x, P = P, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), T)[1])
        @test iszero(AD.derivative(backend, x -> compute_τII(c2, εII, (; T = T, P = x, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), P)[1])
        @test iszero(AD.derivative(backend, x -> compute_εII(c2, τII, (; T = x, P = P, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), T)[1])
        @test iszero(AD.derivative(backend, x -> compute_εII(c2, τII, (; T = T, P = x, d = 100.0e-6, τII_old = 1.0e6, dt = 1.0e8)), P)[1])
    end
