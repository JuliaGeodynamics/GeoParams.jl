...
struct PlotTestDisplay <: AbstractDisplay end
Base.display(::PlotTestDisplay, ::Any) = nothing

# Some plotters deliberately exercise their `display(fig)` branches. Route those calls to a
# test-local sink so running this file never opens an IDE plot pane or an external figure window.
# The display stack is restored even if a plotting assertion fails.
plot_test_display = PlotTestDisplay()
pushdisplay(plot_test_display)
try
    @testset "Plotting (headless / CairoMakie)" begin
        ...
        @testset "stress / strainrate / viscosity" begin
            ...
            # no `filename` -> exercises the `display(fig)` branch through PlotTestDisplay
        ...
        @testset "TAS / Arrhenius / zircon" begin
            @test Plot_TAS_diagram([50.0 5.0; 61.6 6.25]) isa Figure
            Fe = SetChemicalDiffusion(Garnet.Grt_Fe_Chakraborty1992)
