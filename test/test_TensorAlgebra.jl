...
@testset "Tensor algebra" begin
    ...
    @test all(τ_rot .≈ (-0.5680175460828278, 3.068017546082828, 0.8702368655343053))
    # staggered grid
    ...
    # a rigid rotation must preserve the second invariant of the stress tensor
    @test second_invariant(τ_rot...) ≈ second_invariant(τ...)
    # staggered grid
    τyz = 2.5, 2.5, 3.5, 3.5
    τxz = 3.5, 3.5, 4.5, 4.5
    τxy = 4.5, 4.5, 5.5, 5.5
    ...
    @test GeoParams.rotate_elastic_stress3D((0.0, 0.0, 0.0), τ0, dt) == τ0
    τ32 = Float32.(τ0)
    @test eltype(GeoParams.rotate_elastic_stress3D((0.5f0, -0.4f0, 0.2f0), τ32, 1f-3)) == Float32
end
