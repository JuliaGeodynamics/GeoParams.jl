...
@testset "CreepLaw" begin
    ...
    @test isDimensional(x1_D) == true
    @test x1_D.n == 3
    @test x1_D.η0.val == 1.0e24
    @test x1_D.τ0.val == 100.0e6
    @test x1_D.ηr.val == 1.0e20
    @test x1_D.Q.val == 0
    ...
    @test isDimensional(x1_ND) == false
    @test x1_ND.η0 * 1.0 == 1.0e5
    @test x1_ND.τ0 * 1.0 ≈ 10.0
    @test x1_ND.ηr * 1.0 == 1.0e1
    ...
    @test ustrip(τ1273) ≈ 2 * x1_D.η0.val * ustrip(εt) rtol = 1.0e-4
    ε1273 = compute_εII(x1_D, τ1273, (; T = 1273.0K))
    @test εt ≈ ε1273 rtol = 1.0e-6 # see if we can recover the strain rate

    # below yield case - nondimensional
    εr = 0.5 * x1_ND.τ0 / x1_ND.η0 # yield strain rate
    εt = 1.0e-4 * εr # well below the yield

    τ1 = compute_τII(x1_ND, εt, (; T = 1.0))
    @test τ1 ≈ 2 * x1_ND.η0.val * ustrip(εt) rtol = 1.0e-4
    ε1 = compute_εII(x1_ND, τ1, (; T = 1.0))
    @test εt ≈ ε1 rtol = 1.0e-6 # see if we can revocer the strain rate

    εt = [1.0e-4 * εr; 1.0e-4 * εr]
    τ1 = [1.0e3, 1.0e3] # initialize τ1
    compute_τII!(τ1, x1_ND, εt, (; T = ones(size(εt))))
    @test τ1 ≈ [2 * x1_ND.η0.val * ustrip(εt[1]), 2 * x1_ND.η0.val * ustrip(εt[2])] rtol = 1.0e-4
    ...
    @test ustrip(τ1273) ≈ 2 * η_approx * ustrip(εt) rtol = 1.0e-6
    @test ε1273 ≈ εt rtol = 1.0e-6
    #
    #
    x1_ND = nondimensionalize(x1_D, CharUnits_GEO)
    ...
    @test ustrip(nondimensionalize(τ_T1, CharUnits_GEO)) ≈ τ_T1_ND rtol = 1.0e-6
    @test ustrip(nondimensionalize(τ_T2, CharUnits_GEO)) ≈ τ_T2_ND rtol = 1.0e-6
