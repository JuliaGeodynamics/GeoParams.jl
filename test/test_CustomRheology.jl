...
@testset "CustomRheology" begin
    ...
    # in-place array routines (compute_εII! / compute_τII! for CustomRheology)
    sc = (; depth = 1.0e3, P = 1.0e3 * 3300 * 9.81, T = 1.6e3)
    arr_args = (; depth = fill(1.0e3, 4), P = fill(sc.P, 4), T = fill(1.6e3, 4))
    τarr = fill(1.0e3, 4)
    εarr = similar(τarr)
    compute_εII!(εarr, v1, τarr, arr_args)
    @test all(εarr .≈ compute_εII(v1, 1.0e3, sc))
    εin = fill(1.0e-15, 4)
    τout = similar(εin)
    compute_τII!(τout, v1, εin, arr_args)
    @test all(τout .≈ compute_τII(v1, 1.0e-15, sc))
end
