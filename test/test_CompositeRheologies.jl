...
@testset "CompositeRheologies" begin
    ...
    c14 = CompositeRheology(
        SetConstantElasticity(; G=1e10, Kb=2e11),
        LinearViscous(; η=1e20),
        DruckerPrager(; C=3e5, Ψ=10),
    )   # case A
    c15 = CompositeRheology(
        SetConstantElasticity(; G=1e10, Kb=2e11),
        LinearViscous(; η=1e20),
        Parallel(DruckerPrager(; C=3e5, Ψ=10), LinearViscous(; η=1e19Pa * s)),
    )   # case A
    c16 = CompositeRheology(
        SetConstantElasticity(; G=1e10, Kb=2e11),
        LinearViscous(; η=1e20),
        DruckerPrager_regularised(; C=3e5, Ψ=10, η_vp=1e19),
    )   # case A
    ...
    c_ep_reg = CompositeRheology(
        ConstantElasticity(), DruckerPrager_regularised(; η_vp=1e20)
    )
    c_e_vp = CompositeRheology(
        ConstantElasticity(), Parallel(DruckerPrager(), LinearViscous(; η=1e20))
    )
    ...
    c_vep = CompositeRheology(
        LinearViscous(; η=1e22), ConstantElasticity(), DruckerPrager()
    )
    c_vep_reg = CompositeRheology(
        LinearViscous(; η=1e22),
        ConstantElasticity(),
        DruckerPrager_regularised(; η_vp=1e20),
    )
    c_ve_vp = CompositeRheology(
        LinearViscous(; η=1e22),
        ConstantElasticity(),
        Parallel(DruckerPrager(), LinearViscous(; η=1e20)),
    )
