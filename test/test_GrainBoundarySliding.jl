...
@testset "GrainBoundarySliding" begin
    ...
    @test Value(x1.n) == 3.5
    @test Value(x1.p) == -2.0
    @test Value(x1.A) == 1.5MPa^-3.5 * s^-1 * m^(3.0)
    ...
    p = SetDiffusionCreep("Dry Olivine < 1523K | Hirth and Kohlstedt (2003)")
    εII = exp10.(-22:0.5:-12)
    ...
    for itest in 1:2
        ...
        PPa = 0.0
        gsiz = 100
        TK = [(650 + 273.15) (1300 + 273.15)] 

        logA = [3.8129133566428557 10.672097857935718] #Logarithm of pre-exponential factor
        npow = [3.5 3.5] #Power law exponent
        Qact = [400 600] #Activation energy (KJ)
        m_gr0 = [2 2] #Grain size Exponent (will convert to negative)
        Vact = [18 18] #Activation Volume cm-3
        ...
        if itest == 1
            pp = SetDiffusionCreep("Dry Olivine < 1523K | Hirth and Kohlstedt (2003)")
        elseif itest == 2
            pp = SetDislocationCreep("Dry Olivine >= 1523K | Hirth and Kohlstedt (2003)")
        end

        # using SI units
        τII = compute_τII(pp, eII / s, (; T=TK[i_flow] * K, d=gsiz * 1e-6m))
        η = τII / (2 * eII / s)
        @test Tau ≈ ustrip(τII)
        @test mu ≈ ustrip(η)

        εII = compute_εII(pp, τII, (; T=TK[i_flow] * K, d=gsiz * 1e-6m))
        @test eII ≈ ustrip(εII)

        # using Floats
        τII = compute_τII(pp, eII, (; T=TK[i_flow], d=gsiz * 1e-6))
        η = τII / (2 * eII)
        @test Tau ≈ τII
        @test mu ≈ η

        εII = compute_εII(pp, τII, (; T=TK[i_flow], d=gsiz * 1e-6))
        @test eII ≈ ustrip(εII)

        # using arrays for some of the variables
        TK_vec = ones(10) .* TK[i_flow]
    ...
    a =  SetDiffusionCreep("Dry Olivine >= 1523K | Hirth and Kohlstedt (2003)", V=1e-6m^3/mol)
