...
# Rheology
@views function UpdateStressGeoParams!( τxx, τyy, τxy, ε̇xx, ε̇yy, ε̇xy, τxx0, τyy0, τxy0, Pc, MatParam, dt, Phasec, Phasev )
...
@views function Stokes2D_ve()
    ...
    MatParam = (SetMaterialParams(Name="Matrix"   , Phase=1,
                    CompositeRheology = CompositeRheology(ConstantElasticity(G=G),LinearViscous(η=η0),DruckerPrager(C=1.5, ϕ=10))), 
    ...
    ncx, ncy = 3*31, 3*31    # numerical grid resolution
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            # stresses
            UpdateStressGeoParams!( τxx, τyy, τxy, ε̇xx, ε̇yy, ε̇xy, τxx0, τyy0, τxy0, Pt, MatParam, dt, Phasec, Phasev )
            # velocities
        ...
        p4 = heatmap(xc, yc, Pt' , aspect_ratio=1, xlims=(0, Lx), ylims=(dy/2, Ly-dy/2), c=:inferno, title="P")
